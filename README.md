...
**A second, separate SSL requirement this surfaced:** `migrate`/`db push` connect through Prisma's own native schema engine, not through the `mariadb` driver adapter the app uses at runtime (`src/lib/db.ts`) — the two are entirely different codepaths. The schema engine doesn't understand the generic `ssl=true` in `DATABASE_URL`; it needs its own `sslaccept=strict` parameter, and without it, it connects in plaintext, which TiDB Cloud refuses outright (`Connections using insecure transport are prohibited`). `prisma7.config.ts` now adds `sslaccept=strict` automatically whenever `DATABASE_URL` already has `ssl=true` set — nothing to add to the connection string yourself. It's a no-op for a plain local database with no `ssl` param, so local dev without TLS is unaffected.
